...
2. Follow the [conventions in CLAUDE.md](../CLAUDE.md#conventions-every-script-follows):
   ...
   - Defaults via `var=${var:-default}`; locate self with `mydir=...`; call siblings by
     relative path.
3. Add a `README.md` to the directory (purpose, env vars, tests) — match the existing
   ones.
...
and a test tracker. See the [Safety section of CLAUDE.md](../CLAUDE.md#safety-read-before-running-anything).
